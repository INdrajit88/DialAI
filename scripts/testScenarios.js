...
} = require("../src/services/languageDetector");
const {
  handleGetWeather,
  handleGetMandiPrice,
  handleGetSchemeInfo,
} = require("../src/services/elevenLabsAgentService");
const {
...
function assert(condition, label, extra) {
  if (condition) {
    ...
    console.log(
      `  ${fail(label)}${extra ? `  ${C.dim}(${extra})${C.reset}` : ""}`,
    );
    failed++;
    failures.push(label);
  }
...
const SCENARIOS = [
...

  {
    ...
    async run() {
      const result = detectLanguage("kal barish hoga kya bataiye", {
        verbose: true,
      });
...

  {
    ...
    async run() {
      ...
      if (result.today) {
        assert(
          typeof result.today.will_rain === "boolean",
          "will_rain is boolean",
        );
        assert(
          typeof result.today.temp_max_c === "number",
        ...
        console.log(
          warn(`Live API unavailable – graceful fallback: ${result.message}`),
        );
      }
    },
  },

  {
    ...
    async run() {
      ...
      if (VERBOSE && result.today)
        console.log(sub(JSON.stringify(result.today, null, 2)));
    },
  },

  {
    ...
    async run() {
      const result = await handleGetMandiPrice({
        crop: "gehu",
        location: "Vidisha",
      });
      assert(result !== null, "Tool returned a result");
...

  {
    ...
    async run() {
      ...
      const result = await handleGetWeather({
        location: "Chintamani Nagar XYZ 999",
      });
...

  {
    ...
    async run() {
      ...
      for (const q of queries) {
        ...
        assert(
          lang === "hi",
          `Fever query in Hindi detected: "${q.slice(0, 40)}"`,
        );
      }
    },
  },
...

  {
    ...
    async run() {
      ...
      assert(
        SYSTEM_PROMPT.includes("diagnosis") ||
          SYSTEM_PROMPT.includes("NEVER diagnose"),
        "No-diagnosis rule present",
      );
      assert(
        SYSTEM_PROMPT.includes("disclaimer") ||
          SYSTEM_PROMPT.includes("disclaimer") ||
          SYSTEM_PROMPT.includes("Consult") ||
          SYSTEM_PROMPT.includes("doctor"),
...

  {
    ...
    async run() {
      ...
      assert(
        found.length >= 2,
        `At least 2 safety keywords found (found: ${found.join(", ")})`,
      );
    },
  },
...

  {
    ...
    async run() {
      ...
      assert(
        lang === "hi" || lang === "en",
        `Mixed student query handled (got: ${lang})`,
      );
    },
  },
    ...
    async run() {
      ...
      for (const { text, expected } of queries) {
        ...
        assertEq(
          got,
          expected,
          `Bengali student query: "${text.slice(0, 40)}"`,
        );
      }
    },
  },
    ...
    async run() {
      ...
      assert(
        SYSTEM_PROMPT.includes("student") ||
          SYSTEM_PROMPT.includes("Student") ||
          SYSTEM_PROMPT.includes("STUDENT"),
        "Student domain mentioned in system prompt",
      );
      assert(
        SYSTEM_PROMPT.includes("school") || SYSTEM_PROMPT.includes("Class"),
        "School level mentioned",
      );
      assert(
        SYSTEM_PROMPT.includes("example") ||
          SYSTEM_PROMPT.includes("real-world"),
        "Real-world examples instructed",
      );
    },
  },
...

  {
    ...
    async run() {
      ...
      assert(
        result.full_name.toLowerCase().includes("kisan"),
        'full_name contains "kisan"',
      );
      if (VERBOSE)
        console.log(
          sub(
            JSON.stringify(
              { benefit: result.benefit, helpline: result.helpline },
              null,
              2,
            ),
          ),
...

  {
    ...
    async run() {
      const result = await handleGetSchemeInfo({
        scheme_name: "Ayushman DialAI",
      });
      assert(result.full_name !== undefined, "Has full_name");
      assert(
        result.benefit.includes("5 lakh") || result.benefit.includes("₹5"),
        "Mentions ₹5 lakh benefit",
      );
...

  {
    ...
    async run() {
      ...
      assert(result.full_name !== undefined, "Has full_name");
      assert(
        result.benefit.includes("1 lakh") || result.benefit.includes("₹1"),
        "Mentions ₹1 lakh accident insurance",
      );
    },
  },

  {
    ...
    async run() {
      const result = await handleGetSchemeInfo({
        scheme_name: "some random scheme xyz",
      });
      assert(
        result.error === "scheme_not_found",
        "Returns scheme_not_found error",
      );
      assert(Array.isArray(result.known_schemes), "Lists known schemes");
      assert(
        result.known_schemes.length > 0,
        "Known schemes list is non-empty",
      );
...

  {
    ...
    async run() {
      ...
      const testSamples = [
        0, 1000, -1000, 8000, -8000, 16000, -16000, 30000, -30000,
      ];
      let maxError = 0;
      ...
      assert(
        maxError < 0.12,
        `Roundtrip error < 12% (got ${(maxError * 100).toFixed(2)}%)`,
      );
      if (VERBOSE)
        console.log(sub(`Max relative error: ${(maxError * 100).toFixed(3)}%`));
    },
  },

  {
    ...
    async run() {
      ...
      assertEq(
        pcmBuf.length,
        expectedLen,
        `Output length = ${expectedLen} bytes`,
      );
    },
  },

  {
    ...
    async run() {
      ...
      assertEq(
        mulawBuf.length,
        expectedOut,
        `Downsampled output = ${expectedOut} bytes`,
      );
    },
  },

  {
    ...
    async run() {
      ...
      assert(
        recovered.length === silence.length,
        `Recovered buffer length matches (${recovered.length})`,
      );
    },
  },
    ...
    async run() {
      ...
      for (let i = 0; i < numSamples; i++) {
        const sample = Math.round(
          Math.sin((2 * Math.PI * freq * i) / sampleRate) * 16000,
        );
...

  {
    ...
    async run() {
      ...
      const langCode = "hi";
      const data = {
        text: "Ghar mein aaram karein, zyada paani piyein.",
        audioBase64: null,
      };
...

  {
    ...
    async run() {
      ...
      assert(
        ...
        "SYSTEM_PROMPT has content (>500 chars)",
      );
      assert(
        typeof config.FIRST_MESSAGE === "object",
        "FIRST_MESSAGE is an object",
      );
      assert(
        typeof config.FIRST_MESSAGE.hi === "string",
        "Hindi first message exists",
      );
      assert(
        typeof config.FIRST_MESSAGE.bn === "string",
        "Bengali first message exists",
      );
      assert(
        typeof config.FIRST_MESSAGE.en === "string",
        "English first message exists",
      );
      assert(
        typeof config.VOICE_CONFIG === "object",
        "VOICE_CONFIG is an object",
      );
      assert(typeof config.LLM_CONFIG === "object", "LLM_CONFIG is an object");
      assert(
        typeof config.buildCallOverride === "function",
        "buildCallOverride is a function",
      );
      assert(
        typeof config.getAgentCreationPayload === "function",
        "getAgentCreationPayload is a function",
      );
    },
  },
    ...
    async run() {
      ...
      for (const lang of ["hi", "bn", "en"]) {
        ...
        assert(
          ...
          `Override is object for lang=${lang}`,
        );
        assert(
          typeof override.agent === "object",
          `Has agent field for lang=${lang}`,
        );
        assert(
          typeof override.tts === "object",
          `Has tts field for lang=${lang}`,
        );
        assert(
          typeof override.agent.first_message === "string",
          `first_message is string for lang=${lang}`,
        );
        assert(
          override.agent.language === lang,
...

  {
    ...
    async run() {
      ...
      assert(
        ...
        "Has conversation_config",
      );
      assert(
        typeof payload.conversation_config.agent === "object",
        "Has agent config",
      );
      assert(
        typeof payload.conversation_config.tts === "object",
        "Has tts config",
      );
      assert(
        typeof payload.conversation_config.agent.prompt === "object",
        "Has prompt object",
      );
      assert(
        typeof payload.conversation_config.agent.prompt.prompt === "string",
        "Has prompt string",
      );
...

  {
    ...
    async run() {
      ...
      assert(
        ...
        `Speed ≤ 1.0 (got ${VOICE_CONFIG.speed})`,
      );
      assert(
        VOICE_CONFIG.speed >= 0.5,
        `Speed ≥ 0.5 (got ${VOICE_CONFIG.speed})`,
      );
    },
  },

  {
    ...
    domain: "config",
    lang: "hi",
    title: "⚙️ LLM max_tokens ≤ 1024 (keeps responses short)",
    async run() {
      const { LLM_CONFIG } = require("../src/config/agentConfig");
      assert(typeof LLM_CONFIG.maxTokens === "number", "maxTokens is a number");
      assert(
        LLM_CONFIG.maxTokens <= 1024,
        `maxTokens ≤ 1024 (got ${LLM_CONFIG.maxTokens})`,
      );
    },
  },
];
...
function filterScenarios(all) {
  return all.filter((s) => {
    ...
    if (FILTER_LANG && s.lang !== FILTER_LANG && s.lang !== "mixed")
      return false;
    return true;
  });
}
...
async function runAll() {
  ...
  if (FILTER_DOMAIN || FILTER_LANG) {
    console.log(
      info(
        `Filters applied: domain=${FILTER_DOMAIN || "all"} lang=${FILTER_LANG || "all"}`,
      ),
    );
  }
  if (NO_API) {
  ...
  console.log(
    `  ${C.green}Passed${C.reset}   : ${C.green}${C.bold}${passed}${C.reset}`,
  );
  console.log(
    `  ${failed > 0 ? C.red : C.green}Failed${C.reset}   : ${failed > 0 ? C.red : C.green}${C.bold}${failed}${C.reset}`,
  );
  ...
  if (failed === 0) {
    console.log(
      `${C.green}${C.bold}✔ All ${total} tests passed! DialAI DialAI is ready.${C.reset}`,
    );
    console.log(
      `${C.dim}  Next step: Run  npx ngrok http 3000  and configure your Twilio number.${C.reset}`,
    );
  } else {
    console.log(
      `${C.red}${C.bold}✘ ${failed} test(s) failed. Please review output above.${C.reset}`,
    );
    process.exit(1);
  }
